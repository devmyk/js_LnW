...
//	getRecords();
	...
	var isexurl = /^http/i;
	if (isexurl.test(data[curr].fn)) {
		sm = soundManager.createSound({id: data[curr].code , url: data[curr].fn});
	} else {
		// 원래 sm 의 파일명과 같으면 생략
//		if (!sm) {
			sm = soundManager.createSound({id: data[curr].code , url: path + "mp3/" + data[curr].fn + ".mp3"});
